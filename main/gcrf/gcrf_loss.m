    ...
    methods
        ...
        function set_gt(obj, isample, ilocal, label, v) 
            ...
            if numel(label) == 1;
                ...
                %pot = v * (ones(1, size(label, 2)) - label);% / sum(label);
                pot = v * label;
                [~, label] = min(label);
            end
        ...
        function set_gt_pairwise(obj, isample, ilocal, loss, v) 
            % set the ground-truth of a sample
            %
            %   obj.set_gt(isample, ilocal, label);
            %   obj.set_gt(isample, ilocal, label, v);
            ...
            K2 = s.Ks(ilocal(2));
            K = [K1, K2];
            if size(loss, 1) ~= K1 || size(loss, 2) ~= K2
                error('loss is wrong');
            end;
            ...
            obj.loss.sample{isample}.factor{ifac} = ...
                struct('size', K, 'pot', loss, 'nodes', [il1, il2]);

            obj.connect_local(isample, il1, ifac);
            obj.connect_local(isample, il2, ifac);

            obj.fac_counters(isample) = ifac;
        end
        
        function connect_local(obj, isample, ilocal, ifac)              
            v = obj.loss.sample{isample}.local{ilocal};
            if isfield(v, 'connTo')
                v.connTo = [v.connTo, ifac];
            else
                v.connTo = ifac;
            end
            obj.loss.sample{isample}.local{ilocal} = v;
        end     
