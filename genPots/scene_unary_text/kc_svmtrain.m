function [models, clabels] = kc_svmtrain(X, labels, C, gamma, kernel)
...
params = sprintf('-c %g -g %g -t %d -q -d 2', C, gamma, kernel);
...
for k = 1 : K
    ...
    Lcls = L(labels == clabel);
    Xcls = X(labels == clabel, :);
    Lcls = [Lcls; L(ind_neg(r(1:min(length(r), 2*npos))))];
    Xcls = [Xcls; X(ind_neg(r(1:min(length(r), 2*npos))), :)];
...
%         k, K, clabel, npos, nneg);
    ...
    models{k} = svmtrain(Lcls, Xcls, params);
