function [ix, iy, cubeinfo, boxView] = p3d_projcube_s(scene, cube, color)
...
cubeinfo.faces = faces_for_box;
                
[ix, iy] = p3d_caliproj(scene.rot, cubeinfo.vertices(:,1)', cubeinfo.vertices(:,2)', cubeinfo.vertices(:,3)');

boxView = [ix; iy];
vis = get_box_viz2(cubeinfo, boxView);
boxView(3, :) = vis;

%cubeinfo.vertices = [ix'; iy'];
...
hold on;
if isnumeric(color)
    plot(ix, iy, 'Color', color, 'LineWidth', 2);
else
